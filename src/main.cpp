...
void sendToLora2MQTTGateway()
{
  ...
  // increment TxCounter
  Node.TxCounter++;
  // display the 3 lines available for Rx information (user defined)
  u8x8.clearLine(3);
  u8x8.clearLine(4);
  u8x8.clearLine(5);
  u8x8.drawString(0, 3, Node.GetLineToDisplay(1));
  u8x8.drawString(0, 4, Node.GetLineToDisplay(2));
  u8x8.drawString(0, 5, Node.GetLineToDisplay(3));
...
void receiveLoraMessage()
{
  ...
  if (packetSize)
  {
    ...
    // clear error and incoming meessage information lines
    u8x8.clearLine(2);
    u8x8.clearLine(6);
    u8x8.clearLine(7);
    ...
    while (LoRa.available())
    {
      ...
      else
      {
          ...
          if (nodeInvoked.compareTo(Node.GetNodeName()) == 0)
          {
            ...
            u8x8.drawString(0, 6, Node.GetLineToDisplay(4));
            u8x8.drawString(0, 7, Node.GetLineToDisplay(5));
          }
      }
    }
    digitalWrite(LED_WHITE, LOW);
